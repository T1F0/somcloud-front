...
export default {
  methods: {
    ...
    collectDelete(row) {
      axios.delete('http://localhost:8082/concollect/deleteById/'+row.id).then(function (resp)
      {
        alert('商品(id='+row.id+")已取消收藏"),
    ...
    page(currentPage){
      const _this =this
      axios.get('http://localhost:8082/concollect/findAll/'+currentPage+'/10').then(function (resp){                                //调取后端数据,读取到页面相应窗口,+this.$route.query.id
  ...
  created() {
    const _this =this
    axios.get('http://localhost:8082/concollect/findAll/1/10').then(function (resp){                                //调取后端数据,读取到页面相应窗口,+this.$route.query.id
      _this.tableData = resp.data.content                                                       //从content中拿出数据
      _this.total=resp.data.totalElements
      // console.log(resp.data)
    })
