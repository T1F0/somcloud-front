...
export default {
  methods: {
    edit(row) {                                              //row当前这一行的对象
      ...
      row.resourceid                                                 //通过id拿出数据
    },
    carDelete(row) {
      axios.delete('http://localhost:8082/resource/deleteById/'+row.resourceid).then(function (resp)
      {
        alert("数据删除成功"),
  ...
  created() {
    const _this =this
    _this.admin = JSON.parse(window.localStorage.getItem('access-admin'))
    axios.get('http://localhost:8082/resource1/allByName?createuserid='+_this.admin.userid).then(function (resp){                                //调取后端数据,读取到页面相应窗口,+this.$route.query.id
