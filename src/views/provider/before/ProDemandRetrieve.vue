...
export default {
  methods: {
    ...
    carDelete(row) {
      axios.delete('http://localhost:8082/demand/deleteById/'+row.id).then(function (resp)
      {
        alert("数据删除成功"),
    ...
    searchForm(formName){
      ...
      this.$refs [formName].validate((valid) => {
        if (valid) {
          this.$axios.get('http://somc.top:8181/demand/categories/{brand}' + this.ruleForm.brand)
