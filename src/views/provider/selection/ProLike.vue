      ...
      <span>
        ...
        <el-popover placement="top" width="160" v-model="visible">
          ...
          <div style="text-align: left; margin: 0">
            <el-button @click="likeDelete(scope.row)" type="text" size="small">确定</el-button>
            <el-button size="mini" type="text" @click="visible = false">取消</el-button>
          </div>
          <el-button slot="reference">取消喜欢</el-button>
        </el-popover>
      </span>
        </template>
      </el-table-column>
...
export default {
  methods: {
    ...
    init () {
      ...
      this.$axios.get('/book/find')
        ...
        likeDelete(row) {
      axios.delete('http://localhost:8082/prolike/deleteById/'+row.id).then(function (resp)
      {
        alert("商品id=("+row.id+")已取消喜欢"),
    ...
    page(currentPage){
      const _this =this
      axios.get('http://localhost:8082/prolike/findAll/'+currentPage+'/10').then(function (resp){                                //调取后端数据,读取到页面相应窗口,+this.$route.query.id
  ...
  created() {
    const _this =this
    axios.get('http://localhost:8082/prolike/findAll/1/10').then(function (resp){                                //调取后端数据,读取到页面相应窗口,+this.$route.query.id
      _this.tableData = resp.data.content                                                       //从content中拿出数据
      _this.total=resp.data.totalElements
      // console.log(resp.data)
    })
