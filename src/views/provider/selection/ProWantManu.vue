      ...
      <span>
        ...
        <el-popover placement="top" width="160" v-model="visible">
          ...
          <div style="text-align: left; margin: 0">
            <el-button @click="manuDelete(scope.row)" type="text" size="small">确定</el-button>
...
export default {
  methods: {
    ...
    manuDelete(row) {
      axios.delete('http://localhost:8082/prowantmanu/deleteById/'+row.id).then(function (resp)
      {
        alert("商品（id="+row.id+")已移除购物车"),
    ...
    page(currentPage){
      const _this =this
      axios.get('http://localhost:8082/prowantmanu/findAll/'+currentPage+'/10').then(function (resp){                                //调取后端数据,读取到页面相应窗口,+this.$route.query.id
  ...
  created() {
    const _this =this
    axios.get('http://localhost:8082/prowantmanu/findAll/1/10').then(function (resp){                                //调取后端数据,读取到页面相应窗口,+this.$route.query.id
      _this.tableData = resp.data.content                                                       //从content中拿出数据
      _this.total=resp.data.totalElements
      // console.log(resp.data)
    })
